...
document.getElementById('btnConfMov').addEventListener('click', async () => {
  ...
  try {
    if (movTipo === 'entrada') {
      await MovimentacaoAPI.entrada({ produto_id: parseInt(produto_id), estoque_id: parseInt(estoque_id), quantidade: qtd, quantidade_min: qtdMin, observacao: obs, usuario_id: 1 });
      toast('Entrada registrada com sucesso!');
    } else if (movTipo === 'saida') {
      await MovimentacaoAPI.saida({ produto_id: parseInt(produto_id), estoque_id: parseInt(estoque_id), quantidade: qtd, observacao: obs, usuario_id: 1 });
      toast('Saída registrada com sucesso!');
    } else {
      await MovimentacaoAPI.transferencia({ produto_id: parseInt(produto_id), estoque_origem_id: parseInt(estoque_id), estoque_destino_id: parseInt(document.getElementById('iMovDestino').value), quantidade: qtd, observacao: obs, usuario_id: 1 });
