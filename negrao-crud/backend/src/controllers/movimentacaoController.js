...
exports.listar = async (req, res) => {
  try {
    ...
    const [rows] = await db.query(`
      ...
      FROM Movimentacao m
      INNER JOIN Produto  p  ON p.idProduto  = m.produto_id
...
exports.entrada = async (req, res) => {
  const { produto_id, estoque_id, quantidade, quantidade_min = 0, observacao, usuario_id = 1 } = req.body;

  if (!produto_id || !estoque_id || !quantidade || quantidade <= 0)
    return res.status(400).json({ erro: 'produto_id, estoque_id e quantidade são obrigatórios.' });
  ...
  try {
    ...
    await db.query(
      'INSERT INTO Movimentacao (produto_id, usuario_id, tipo, quantidade, estoque_destino_id, observacao) VALUES (?, ?, ?, ?, ?, ?)',
      [produto_id, usuario_id, TIPO_ENTRADA, quantidade, estoque_id, observacao || null]
    );
...
exports.saida = async (req, res) => {
  const { produto_id, estoque_id, quantidade, observacao, usuario_id = 1 } = req.body;
  ...
  try {
    ...
    await db.query(
      'INSERT INTO Movimentacao (produto_id, usuario_id, tipo, quantidade, estoque_origem_id, observacao) VALUES (?, ?, ?, ?, ?, ?)',
      [produto_id, usuario_id, TIPO_SAIDA, quantidade, estoque_id, observacao || null]
    );
...
exports.transferencia = async (req, res) => {
  const { produto_id, estoque_origem_id, estoque_destino_id, quantidade, observacao, usuario_id = 1 } = req.body;

  if (!produto_id || !estoque_origem_id || !estoque_destino_id || !quantidade || quantidade <= 0)
    return res.status(400).json({ erro: 'produto_id, estoque_origem_id, estoque_destino_id e quantidade são obrigatórios.' });
  ...
  try {
    ...
    await db.query(
      'INSERT INTO Movimentacao (produto_id, usuario_id, tipo, quantidade, estoque_origem_id, estoque_destino_id, observacao) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [produto_id, usuario_id, TIPO_TRANSFERENCIA, quantidade, estoque_origem_id, estoque_destino_id, observacao || null]
    );
